...
\begin{theorem}[Global optimality for convex problems]
 Let $f: \reals^n \to \reals$ be a convex function, that is, $f(\lambda x_1 + (1-\lambda)x_2) \le \lambda f(x_1) + (1-\lambda)f(x_2), \ \lambda \in [0,1]$, and let $S \subset \reals^n$ be a convex set. Let $\textbf{x}^{*}$ be an element of $S$. Suppose that $\textbf{x}^{*}$ is a local optimum for the problem of minimising $f(\textbf{x})$ over $S$. That is, there exists some $\epsilon > 0$ such that $f(\textbf{x}^{*}) \leq f(\textbf{x})$ for all $\textbf{x} \in S$ for which $\|\textbf{x} - \textbf{x}^{*}\| \leq \epsilon$. Then, $\textbf{x}^{*}$ is globally optimal, meaning that $f(\textbf{x}^{*}) \leq f(\textbf{x})$ for all $\textbf{x} \in S$. 
\end{theorem}
