...
\begin{enumerate}
	...
	\item If a proper subspace has dimension $m < n$, then it means that there are $n-m$ directions in $\reals^n$ that are perpendicular to the subspace. That is, there are nonzero vectors $a_i$ that are orthogonal to $S$. Or, equivalently, $a_i^\top x = 0$ for $i = n-m + 1, ..., n$. Referring to the $\reals^3$, if $m=2$, then there is a third direction that is perpendicular to (or not in) $S$. Figure \ref{p1c2:fig:proper_subpaces} illustrates this.
\end{enumerate}
...
\input{part_1/chapter_2/exercises/E23-actconst}

\subsection*{Exercise 2.4: Vertex, extreme points, and BFSs}
\input{part_1/chapter_2/exercises/E24-bfs}

\subsection*{Exercise 2.5: Binding constraints applications} 
\input{part_1/chapter_2/exercises/E25-binding}
