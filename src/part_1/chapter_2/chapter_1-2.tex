...
\begin{theorem}[Forming bases from LI vectors]\label{p1c2:thm:LI_and_bases}
...
Definition \ref{p1c2:def:polyhedral_sets} formally state the structure we refer to as polyhedral sets.
%
...
Let us first define the notion of convexity for sets, which is stated in Definition X
...
We are now ready to state the result that guarantees that polyhedral sets of the form
$$
...
Figure \ref{p1c2:fig:convexity_theorem_examples} illustrate some of the statements represented in the proof. For example, the intersection of the convex sets is always a convex set. One should notice however that the same does not apply to the union of convex sets. Notice that statement 2 proves that polyhedral sets as defined according to Definition \ref{p1c2:def:polyhedral_sets} are convex. Finally the third figure on the right illustrate the convex hull of four points as a convex polyhedral set containing the lines connecting any two points within the set. 
