...
We now will consider problems in which we have the presence of integer variables. As we will see in the next chapter, the inclusion of integrality requirements impose computational and theoretical challenges that we must overcome to be able to solve these problems. On the other hand, being able to consider integer variables allows for the modelling of far more complex and sophisticated systems, to an extent that integer programming problems, or more specifically, mixed integer programming problems are by far the most common in practice.

As we did in the first chapter, we will stat our discussion presenting general problems which have particular structures that can often be identified in larger and more realistic settings. This will also be helpful in exemplifying how integer variables can be used to model particular features of optimisation problems.

Let us first specify what do we mean by integer programming problem. Our starting point is a linear programming problem:
  %
  \begin{align*}
	  \text{(P)} : \mini \ &c^\top x \\
	  \st & Ax \leq b \\
...
\end{align*}
%
where $c_j$, $j \in N$, is a weight, and $\mathcal{F}$ is a family of feasible subsets. As the name suggests, in these problems we are trying to form combinations of elements such that a measure (i.e., an objective function) is optimised. Integer programming happens to be an important framework in expressing combinatorial optimisation problems, though both integer programming and combinatorial optimisation expand further to other settings as well. To see this connection, let us define an \emph{incidence vector} $x^S$ of $S$ such that
%
  \begin{align*}
	  x^S_j = \begin{cases}
	       1, \text{if } j \in S \\ 
...
Also, the knapsack problem is also a combinatorial optimisation problem, which can be stated by making $N$ the set of all items $\braces{1,\dots, n}$, $S \in \mathcal{F}$ the subset of items with total cost not greater than $B$, and $x^S$ such that $x_{i}, \forall i \in N$.   


\subsection{The generalised assignment problem}

The generalised assignment problem (or GAP) is a generalisation of the assignment problem including a structure that resembles that of a knapsack problem. In this case, we consider the notion of bins, to each the items have to be assigned to. In this case multiple items can be assigned to a bin, or a bin might have no item assigned. In some context, this is problem is also known as the bin packing problem. 
...
Hopefully, the parallel to the combinatorial optimisation problem version is clear at this point and is let for the reader as a though exercise.


\subsection{The set covering problem}

The covering problems relate to the location of infrastructure with the objective of covering demand points and, thus, a frequently recurring in settings where service centres such as fire brigades, hospitals, or police stations must be located to efficiently serve locations.
...
Figure \ref{p1c8:fig:set_covering} illustrates an example of a set covering problem based on a fictitious map broken into regions. Each of the cells represent regions that must be covered, i.e., $M = \braces{1,\dots, 20}$. The blue cells represent regions that can have a centre opened, that is, $N = \braces{3,4,7,11,12,14,19}$. Notice that $N \subset M$. In this case, we assume that if a centre is opened at a blue cell, then it can serve all adjacent cells. Therefore, we have, e.g., that $S_3 = \braces{1,2,8}$, $S_4=\braces{2,5,6,7}$, and so forth.
...
To model the set covering problem, and pretty much any other problem involving indexed subsets such as $S_j$, $\forall j \in N$, we need an auxiliary structure, often referred to as \emph{0-1 incidence matrix} $A_{ij} = 1$ if $i \in S_j$, $A_{ij} = 0$ otherwise. For example, referring to Figure \ref{p1c8:fig:set_covering}, the first column of $A$ would refer to $j = 3$ and would have nonzero values at rows 1, 2, 3, and 8.

We are now ready to pose the set covering problem as an integer programming problem. For that, let $x_{j} = 1$ if facility is opened at location $j$ and $x_j = 0$, otherwise. In addition, let $M = \braces{1,\dots,m}$ be the set of regions to be served and $N = \braces{1,\dots,n}$ the set of candidate places to have a centre opened. Then, the set covering problem can be formulated as follows.
%
\begin{align*}
...
\begin{figure}[h]
	...
	\begin{tikzpicture}[node/.style={circle, fill=blue!60, draw, minimum size=1em, inner sep=2pt}, scale=1]
	    ...
	    \draw[->, thick] (5) -- (1);
	\end{tikzpicture}
	\caption{An example of a tour between the six cities, starting from city 1} \label{p1c8:fig:TSP}
\end{figure}

To pose the problem as an integer programming model, let us define $x_{ij} =$ 1 if city $j$ is visited directly after city $i$, $x_{ij} =  0$ otherwise. Let $N = \braces{1,\dots,n}$ be the set of cities. We assume that $x_{ii}$ is not defined for $i \in N$. A naive model for the travelling salesperson problem would be 
%
\begin{flalign*}
...
\end{flalign*}
%
First, notice that the formulation of $TSP$ is exactly the same as that of the assignment problem, however, this formulation has an issue. Although it can guarantee that all cities are only visited once, it cannot enforce an important feature of the problem that is that the tour cannot contains \emph{sub-tours}. In other words, the salesperson must physically visit from city to city in the tour, and cannot ``teletransport'' from one city to another. Figure \ref{p1c8:fig:TSP_subtours} illustrate the concept of sub-tours.
...
\begin{figure}[h]
	...
	\begin{tikzpicture}[node/.style={circle, fill=blue!60, draw, minimum size=1em, inner sep=2pt}]
		...
		\draw[->] (6) -- (4);
	\end{tikzpicture}
	\caption{A feasible solution of for the naive TSP model. Notice the two sub-tours formed} \label{p1c8:fig:TSP_subtours}
\end{figure}

In order to prevent subtours, we must include constraints that can enforce the full connectivity of the tour. There are mainly two types of such constraints. The first is called \emph{cut-set constraints} and are defined as
%
\begin{equation*} 
...
An alternative type of constraint is called \emph{subtour elimination} constraint and is of the form
%
\begin{equation*}
...
\begin{figure}[h]
	...
	\end{subfigure}
	\caption{An illustration of the facility location problem an one possible solution with two facilities located}	
\end{figure}
...
An LP relaxation simply consists of a version of the original MIP problem in which the integrality requirements are dropped. All the methods used to solve MIP models are based on LP relaxation. 
...
\begin{definition} \label{p1c8:def:formulation}
	A polyhedral set $P = \braces{x \in \reals^{n+p} : Ax \leq b}$ is a \emph{formulation} for a set $X \subseteq \integers^n \times \reals^p $ if and only if $X = P \cap (\integers^n \times \reals^p)$.
\end{definition}

One aspect that can be noticed from Definition \ref{p1c8:def:formulation} is that a given collection of points representing the feasible region of a MIP problem, represented by $X$. This is illustrated in Figure \ref{p1c8:fig:alternative_formulations}, where one can see three alternative formulations for the same set $X$.
...
The formulation $P_3$ has a special feature associated to it. Notice how all extreme points of $P_3$ belong to $X$. This has an important consequence. That implies that the solution of the original integer programming problem can be obtained by solving a single LP relaxation, for the solution of both problems is the same. This is precisely what characterises an \emph{ideal} formulation, which is that leading to a minimal (i.e., only once) number of required LP relaxation solutions as solving an LP relaxation over an ideal $P$ yields a solution $x \in X$ for any cost vector $c$. This will only be the case if the formulation $P$ is the \emph{convex hull} of $X$.
