...
Now to the most remarking result, which ties together interior point methods and barrier methods for linear programming problems. If we derive the Karush-Kuhn-Tucker optimality conditions (Section \ref{p2c7:sec:KKT} in the nonlinear optimization section of these notes) for $P_\mu$, we obtain the following set of conditions
	% 
	\begin{align*}
		&Ax = b, \ x > 0\\ 
