...
We will now devise a method to solve mixed-integer programming problems that relies on the successive solution of linear programming relaxations. Although there are several methods that can be employed to solve combinatorial optimisation problems, most of than are not  capable of proving optimality of the solution obtained (i.e., are heuristics) or do not exploit the availability of a (linear) mathematical programming formulation. To date, the only method capable of both is something that is generally known as a \emph{branch-and-cut} method.

Branch-and-cut methods are composed of a combination of multiple parts, including, among other things, a branch-and-bound approach, cutting planes, and heuristics as well. In the next chapters we will focus on each of these parts individually, starting with the branch-and-bound method.
...
\end{equation*}
%
To prove that a given solution $x*$ is optimal, we must rely on the notion of bounding. That is, we must provide a pair of upper and lower bounds that are tight as possible. In the occasion that these bounds are the same, and thus match the value of $z = c^\top x^*$, we have available a certificate of optimality of $x^*$. This concept must be familiar to you. We already used a similar argumentation in Chapter \ref{chapter_5}, when we introduced the notion of dual bounds.

Most methods that can prove optimality work by \emph{bounding} an optimal solution. In this context, bounding means to construct an increasing sequence of lower bounds
%
\begin{equation*}
...
Primal bounds can be obtained by obtaining a feasible solution. For example, by heuristically assembling a solution that is feasible by construction. On the other hand, dual bounds are typically obtained by means of solving a \emph{relaxation} of the original problem. We are ready now to provide Definition \ref{p1c9:def:relaxation}, which formally defines the notion of a relaxation.
...
\begin{proposition} \label{p1c9:prop:relaxation_optimality}
  ...
  \begin{enumerate}
  	\item If a relaxation RP is infeasible, then IP is infeasible. 
  	\item Let $x^*$ be an optimal solution for RP. If $x^* \in X$ and $\overline{c}^\top x^* = c^\top x^*$, then $x^*$ is an optimal solution for IP.
  \end{enumerate}
\end{proposition}
%
\begin{proof}
...
In the context of solving (mixed-)integer programming, we will rely on the notion of linear programming (LP) relaxations. We have briefly discussed the idea in the previous chapter, but, for the sake of precision, let us first define what we man by the term. 
...
\begin{proposition} \label{p1c9:prop:tighter_relaxations}
	Let $P_1$ and $P_2$ be formulations of IP problem $\mini_x \braces{c^\top x : x \in X}$ with $X = P_1 \cap \integers^n = P_2 \cap \integers^n$. Assume $P_1$ is a better formulation than $P_2$ (i.e., $P_1 \subset P_2$). Let $z_{LP}^i = \mini_x \braces{c^\top x : x \in P_i}$ for $i=1,2$. Then $z_{LP}^1 \geq z_{LP}^2$ for any cost vector $c$.
\end{proposition}
...
There is another important type of relaxation that is often exploited in the context of combinatorial optimisation. Specifically, a relaxation for a combinatorial optimisation problem which is also a combinatorial optimisation problem is called a \emph{combinatorial relaxation}.

Efficient algorithms are known for some combinatorial optimisation problems, and this can be exploited in a solution method for problem to which the combinatorial relaxation happens to be such. 

Let us illustrate the concept with a couple of example. Consider the travelling salesperson problem (TSP). Recall that, without considering the tour elimination constraints, we recover the assignment problem. It so turns out that the assignment problem can be solved efficiently (see the Hungarian method) and thus can be used as a relaxation for the TSP, i.e.,
%
\begin{align*}
...
Now, notice the following: (i) a tour contains exactly two edges adjacent to the origin node (say, node 1) and a path through nodes $\braces{2, \dots, |V|}$l; (ii) a tour is a special case of a \emph{(spanning) tree}, which is any subset of edges that covers (or touch at least once) all nodes $v \in V$. 
...
The preceding discussion concerning relaxations, linear programming or combinatorial, represents a major step towards solving mixed-integer programming and/or combinatorial optimisation problems. However, the relaxations are only part of the framework (specifically, the bounding part of the branch-and-bound method). One still needs to be able to somehow, from the solution of said relaxations, be able to construct a solution to the original problem.

\emph{Branch-and-bound} is an algorithmic strategy that is far wider than mathematical programming and optimisation. In essence, it consists of a \emph{divide-and-conquer} strategy, in which we first break an original problem into smaller and manageable (or solvable) parts and then recombine the solution of these parts into a solution for the original problem.

Specifically, let 
%
\begin{equation*}
	P: z_{IP} = \maxi_x \braces{c^\top x : x\ \in S}. 
\end{equation*}
%
...
\begin{proposition} \label{p1c9:prop:divide-and-conquer}
	...
	%
    \begin{equation*}
    	z_{IP} = \max_{k \in K} \braces{z^k}.	
    \end{equation*}
\end{proposition}

Notice the use of the word \emph{decomposition} in Proposition \ref{p1c9:prop:divide-and-conquer}. Indeed, the principle is philosophically the same, and this connection will be exploited in later chapters when we discuss in more details technology for solving mixed-integer programming problems.
...
\subsection{Linear-programming-based branch-and-bound for integer-programming problems}
...
The characteristics that define a branch-and-bound method are thus the relaxation being solved (or how bounding is performed), and how subproblems are generated (how branching is performed). In the specific context of integer programming problems, bounding is performed utilising linear programming (LP) relaxations. 
...
Figure \ref{p1c9:fig:prune_by_optimality} illustrates the process of pruning by optimality. Each box denotes a subproblem, with the interval denoting known lower (primal) and upper (dual) bounds for the problem and $x$ denoting a solution for a relaxation of the subproblem. In Figure \ref{p1c9:fig:prune_by_optimality}, we see a pruning that is caused because a solution to the original subproblem has been identified by solving its relaxation, akin to the leaves in the enumerative tree represented in Figure \ref{p1c9:fig:binary_tree}. This can be attested because the solution of subproblem $S_1$ is integer. 
...
Also, recall that we have seen how to efficiently resolve a linear programming problem from an optimal basis once we include an additional constraint. It so turns out that an efficient dual simplex    method is the kingpin of an efficient branch-and-bound method for (mixed)-integer programming problems.

Finally, although we developed the method in the context of integer programming problems, the method can be readily applied to mixed-integer programming problems, with the only difference being that the branch-and-bound steps are only applied to the integer variables while the continuous variables are naturally taken care in the LP relaxations.
...
Notice that, at this point, our list of active subproblems is formed by $\mathcal{L} = \braces{S_1, S_{11}, S_{1,2}}$. Our current branch-and-bound tree is represented in Figure \ref{p1c9:fig:example_bb_tree_2}. 
...
Notice that in this example, the order in which we solved the subproblems was crucial for pruning by bound the subproblem $S_{11}$, only possible because we happened to solve the LP relaxation of $S_{12}$ first and that happened to yield a feasible solution and associated primal bound. This illustrates an important aspect associated with the branch and bound method: having good feasible solutions available early on in the process increases the likelihood of performing more pruning by bound, which is highly desirable in terms of computational savings (and thus, performance). We will discuss more deeply the impacts of different search strategies later on when we consider this and other aspects involved in the implementation of mixed-integer programming solvers.
